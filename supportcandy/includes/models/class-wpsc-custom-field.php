	...
	final class WPSC_Custom_Field {
		...
		public static function apply_schema() {
			...
			$schema = array(
				...
				'number_type'       => array( // Used to set whether current field is integer or float.
					'has_ref'          => false,
					'ref_class'        => '',
					'has_multiple_val' => false,
				),
		...
		public static function find( $filter = array(), $is_object = true ) {
			...
			$sql   = 'SELECT * FROM ' . $wpdb->prefix . 'psmsc_custom_fields ';
			...
			// total results.
			$sql = 'SELECT count(id) FROM ' . $wpdb->prefix . 'psmsc_custom_fields ';
