...
if ( ! class_exists( 'WPSC_ACB_Messages' ) ) :

	final class WPSC_ACB_Messages {
		...
		public static function apply_schema() {
			...
			self::$schema = apply_filters( 'wpsc_acb_messages_schema', $schema );

			// Prevent modify.
			$prevent_modify       = array( 'id', 'session_id' );
			self::$prevent_modify = apply_filters( 'wpsc_acb_messages_prevent_modify', $prevent_modify );
		}
		...
		public function __construct( $id = 0 ) {
			...
			if ( $id > 0 ) {
				$tag = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}psmsc_acb_messages WHERE id = %d", $id ), ARRAY_A );
				if ( ! is_array( $tag ) ) {
					return;
				}
		 ...
		 * @return boolean|WPSC_ACB_Messages
		 */
		public static function insert( $data ) {
			...
			$log = new WPSC_ACB_Messages( $wpdb->insert_id );
		 ...
		 *
		 * @param WPSC_ACB_Messages $log - log object.
		 * @return boolean
		 */
		...
		public static function find( $filter = array(), $is_object = true ) {
			...
			$sql   = 'SELECT acbm.* FROM ' . $wpdb->prefix . 'psmsc_acb_messages acbm ';
			$where = self::get_where( $filter );

			// Add table alias to orderby.
			$filter['orderby'] = 'acbm.' . $filter['orderby'];

			$order = WPSC_Functions::parse_order( $filter );
			$limit = WPSC_Functions::parse_limit( $filter );

			$group_by = 'GROUP BY acbm.id ';

			$sql = $sql . $where . $group_by . $order . $limit;
			$results     = $wpdb->get_results( $sql, ARRAY_A );

			// total results.
			$sql = 'SELECT count(DISTINCT acbm.id) FROM ' . $wpdb->prefix . 'psmsc_acb_messages acbm ';
		...
		public static function load_ref_class( $classes ) {
			...
			$classes['wpsc_acb_messages'] = array(
				'class'    => __CLASS__,
				'save-key' => 'id',
			);
			return $classes;
